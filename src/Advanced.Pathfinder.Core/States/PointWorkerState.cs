...
using Advanced.Pathfinder.Core.Persistence.Data;
using Advanced.Pathfinder.Core.Persistence;
using Advanced.Pathfinder.Core.Configs;

namespace Advanced.Pathfinder.Core.States;
...
public record PointWorkerState
{
    ...
    internal bool UpdatePointCost(PointCommit commit) => ((ICommit)commit).ChangeMethod switch
    {
    ...
    public bool TryLoopDetection(FindPathRequest request)
    {
        ...
        if (loopDetectionList.Exists(x => x.PointId.Equals(PointId))) return true;
        return false;
    }
